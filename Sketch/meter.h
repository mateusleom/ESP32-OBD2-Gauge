...
void plotNumeric(uint8_t cell, float data, int warn, bool digit) {
  int compare;
      if (cell == 3){
      compare = data/warn;// usar esse quando tiver o sensor
      //compare = warn/data; //usar esse enquanto não tiver o sensor
  } else{
    compare = warn/data;}//compare data and warn  
  ...
  if (compare == 0) {//warning
    ...
    tft.setTextColor(TFT_WHITE, TFT_BLACK);//green color font
    tft.drawRightString(result.c_str(), x + w - 5, y + (h * 0.25) + 11, 6);
    old_data[pidIndex] = data;//update old data
...
void arcMeter(int cell, byte pid) {
  ...
  tft.drawString("---", x + 10, y + 60, 4);  // data
  //draw arc animation
...
void updateMeter(uint8_t pidNo, String response) {  //update parameter on screen
  ...
  switch (formula) {                  //choose fomula
    ...
    case 6: data = (A/1.28)-100; break;
    case 7: data = (analogRead(ADC_PIN)-307)/(139*1.5) ; break;//((analogRead(ADC_PIN))/41);data=data/10.0f; break;
